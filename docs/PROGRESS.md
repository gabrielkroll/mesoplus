...
| S-RemoveSession — Undo toast + Sheets delete | `313a775` | "Remove session" in tday and csheet dot menus now shows a persistent global toast: "[Type] removed · ↩ Undo · ×". ↩ restores exact snapshot (tday: loadSess; csheet: pushes activity back, rebuilds session). × or nav-away fires Sheets cleanup: deleteFromSheets(date) if session is empty, syncDayToSheets if readiness/notes remain. beforeunload uses keepalive:true. syncDayToSheets now calls deleteFromSheets when session missing/empty — closed the gap where removed sessions lingered in Sheets forever. Flush hooks in nav(), shiftWeek(), visibilitychange, beforeunload. |
