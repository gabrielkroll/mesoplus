...
- **S-TrainCards notch geometry (2026-05-13)**: Figma ratio node `234:2065` defines notch widths with golden-ratio/Fibonacci proportions (`430 → 266 → 164 → 102`). Runtime SVG geometry now sets the top notch apex to `width / phi^3` and the bottom-right notch apex offset to `width / phi^2`; depth and corner radius stay fixed px tokens until a height-ratio AC exists. The Readiness card now uses the shared golden-ratio path builder too, so all image-card faces follow the same notch ratio system.
