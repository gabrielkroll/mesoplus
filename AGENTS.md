...
| AI | Codex API (`Codex-sonnet-4-6`) direct from browser |
...
├── .gitignore          # Ignores .Codex/, serve.sh, mesoplus.html
├── AGENTS.md           # This file
└── docs/
...
Only commit `index.html` (and docs). Never commit `.Codex/`, `serve.sh`, or `mesoplus.html`.
...
## Future Obsidian/Kanban Note

Gabriel plans to bring back the Shape Up → Kanban column model on Tuesday evening. Do not infer or invent the final columns before then.

Important idea to preserve: Obsidian vaults may need explicit privacy levels. Treat vault content as belonging to different privacy tiers before building Kanban automation, linking, summaries, AI review, or cross-vault workflows. Future design should define which vaults/notes are private, project-shareable, Codex/Codex-readable, or publishable, and make those boundaries visible in the workflow.
...
- Do not commit `.Codex/`, `serve.sh`, or `mesoplus.html`
